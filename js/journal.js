...
function Writer(htmlid, log, rateMultiplier){
  ...
  function addNextChar(){
    ...
    if (!strArrayChar) { console.log('undefined keycode ' + log[index][0]  );index++;return; }
    ...
    if (currentKeycode == 8) {
      ...
      if (strArray.length == strArrayIdx) {
        ...
        if (strArrayIdx == 0) {index++; console.log('backspace at 0');return; }
      ...
      //Delete character in middle of strArray
        ...
        //console.log("splice: "  + strArray.join(''));
...
function journalListener(htmlId){
  ...
  this.startListening = target.addEventListener('keydown', function(e){
    ...
    if (keycode == 16 && startPos == endPos) {lastKeycodeWasShift = true; return;}
    ...
    console.log("code: "+ keycode + " Value: " + String.fromCharCode(keycode)
